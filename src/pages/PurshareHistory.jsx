...
export const PurshareHistory = () => {
	...
	return (
		<div className='container mx-auto p-4 bg-white transition-colors duration-300'>
			<h1 className='text-3xl font-bold mb-6 text-gray-800 '>
				История покупок
			</h1>
			{orders.length === 0 ? (
				<p className='text-gray-800 '>Заказы отсутствуют.</p>
			) : (
				orders.map(order => (
					<div
						key={order.id}
						className='mb-6 p-4 bg-white shadow rounded transition-colors duration-300'
					>
						...
						<h3 className='font-bold mt-4 mb-2 text-gray-800 '>Товары:</h3>
